...
仔细阅读当前项目的代码， think untra hard， 构建一个更新的 design doc， 放在./specs/0004-design.md 文件中, 输出为中文, 使用 mermaid 绘制架构、设计、组件、流程等图表并详细说明。


## 规则测试需求

基于现有的功能，参考./specs/0004-design.md和现有代码，我想增加一个规则更新和攻击测试的需求，请帮我生成需求和设计文档，放在./specs/0005-rule-spec.md 

要求如下：
1、下载ET Open最新的规则，对下载的规则进行分析并分类，对比现有规则，增量更新至现有规则中，并支持下发至探针，支持规则的版本管理。
2、针对每条规则提供发起攻击的按钮，攻击要基于现有的探针服务，发起攻击后可以验证攻击是否成功，查看云端是否有对应日志，支持规则和对应日志的关联；支持批量和单条的攻击；