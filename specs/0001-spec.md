...
*   **高性能**: Probe Manager 使用 epoll 实现事件驱动架构，支持高并发管理多个探针。
...
sequenceDiagram
    ...
    loop 每 5 分钟
        PM->>C: POST /api/probe/heartbeat<br/>{probe_id, rule_version, probes_status}
        C->>PM: {status, latest_rule_version}

        alt 有新规则版本
            PM->>C: GET /api/rules/download/{version}
            C->>PM: 规则文件内容
            PM->>PM: 保存规则文件
    ...
    loop 实时
        SR->>NP: eve.json 日志
        NP->>PM: EVT_ALERT (TCP Socket)
        PM->>PM: 聚合缓冲
    end

    loop 批量上报
        PM->>C: POST /api/probe/logs<br/>{probe_id, logs[]}
        C->>PM: {status: ok}
    end
```
...
1.  **探针注册 (Probe -> Cloud)**
    ...
    *   Request: `{ "probe_id": "...", "name": "...", "ip": "...", "probe_types": ["nids"] }`
...
2.  **心跳/规则检查 (Probe -> Cloud)**
    ...
    *   Request:
        ```json
        {
            ...
            "probes": [
                { "type": "nids", "id": "nids-001", "status": "running", "interface": "eth0" }
            ]
        }
        ```
...
3.  **日志上报 (Probe -> Cloud)**
    *   `POST /api/probe/logs`
    *   Request: `{ "probe_id": "...", "logs": [ { "probe_type": "nids", ...eve_log... }, ... ] }`
...
flowchart LR
    ...
    subgraph 探针节点
        E[Manager: 心跳检查] --> F{版本比较}
        F -->|有更新| G[下载新规则]
        G --> H[保存到本地]
        H --> I[发送 RELOAD 命令]
        I --> J[NIDS 探针]
        J --> K[kill -USR2 Suricata]
        K --> L[Suricata 重载规则]
        F -->|无更新| E
        L --> E
    end
...
erDiagram
    ...
    PROBE_NODE ||--o{ ALERT_LOG : generates
    PROBE_INSTANCE ||--o{ ALERT_LOG : produces
```
